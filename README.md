...
車は車庫とつながる前庭の黄色い駐車枠から出発し、車種は道具の絵で選べます。車庫は柱・壁・段屋根がつながった形で、三色旗は道路脇の支柱に立っています。操作説明は画面幅に合わせて全文表示します。
車庫の中、または前庭の駐車枠で停止すると5車種の選択が表示されます。乗り換えた車は前庭の駐車枠から出発します。
消防車はつながる水流と着水のしぶき、救急車はハートとケアの波、パトカーは左右のサイレン波で操作に反応します。救急車の十字とパトカーの灯火は車体に固定し、明るさで動作を示します。パトカーは窓と車室の高さを確保した形です。
救急車とショベルカーは対象へ実際に作用している間だけゲージが進み、完了すると仕事札にチェックが付きます。
通常のボタンはTabで選びSpace/Enterで操作できます。道具ボタンではSpace/Enterを押している間だけ道具を使います。
読み込みが長引いたときや3D表示が止まったときは「もういちど ひらく」で同じURLを再試行できます。進捗は最初からになります。
[初見体験の設計](docs/design/2026-09-12-product-usability-review.md)と[街・車体の再構築](docs/design/2026-09-13-town-rebuild/README.md)に詳細を記載しています。広い画面では案内を左右の運転ボタンの間に配置し、前庭と車庫を一緒に見渡せます。
...
仕事札には、仕事名に加えて「何へ・どの操作をするか」と`クリア 現在/目標`が表示されます。右上の小さな「おしごとマップ」では、選んだ車色の現在地、黄色い次ターゲット、目的地までの距離を確認できます。対象を完了すると次の未完了対象へピンが移り、仕事完了後は中央車庫を示します。車庫へ戻ると次の仕事が始まり、消防車の炎も再び表示されます。

中央の黄色い駐車枠へ戻って停止すると「しょうぼうしゃ」「ブルドーザー」「ショベルカー」「きゅうきゅうしゃ」「パトカー」を選べます。選択に失敗や解除条件はなく、車庫へ戻れば何度でも乗り換えられます。消防車の主操作は放水、ブルドーザーは前面ブレード、ショベルカーはバケット、救急車は手当て、パトカーはサイレンです。
