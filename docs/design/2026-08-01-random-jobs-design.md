...
> 2026-08-03の3歳児向け視認性レビューにより、建物北面の窓2件は屋外2地点へ置換した。最新の配置原則と要件差分は`2026-08-03-preschool-mission-visibility-design.md`を正とする。

| job ID | 仕事札 | 炎の照準点 | 現場 |
| --- | --- | --- | --- |
| `fire-side` | よこの火をけそう | 既存の東側面 | 火災地区の家の右側 |
| `fire-hydrant` | しょうかせんのそばをけそう | 消火栓の東側 | 火災地区入口の開けた床 |
| `fire-planter` | おはなのそばをけそう | 花壇の中心 | 東側道路から見える花壇 |
