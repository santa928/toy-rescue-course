...
/** 3台selectorと主要HUDが画面内・親内・安全余白内にあることを実寸で検証する。 */
async function measureFleetHud(page, viewport) {
  ...
  assert.equal(buttonBoxes.length, 3, `${viewport.name}: selector must contain three vehicle buttons.`);
...
async function digTarget(page, viewport, expectedCompletedCount) {
  ...
  assert(completed,
    `${viewport.name}: soil target ${expectedCompletedCount} did not complete within 150 frames.`);
  return completed;
}
